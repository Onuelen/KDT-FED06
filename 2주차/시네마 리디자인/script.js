...
// 1_1_1.웹 브라우저는 DOM 객체 모델을 갖고 있으니깐 해당 DOM에서 card_iteme들의 Node를 찾아와야 겠다.

//1_1_1_1.DOM에서 원하는 Node를 찾아오려면 querySelector() 혹은 getElementById()들의 함수를 사용할 수 있다.

// 1_2.card_item이 1개 아니라, 총 5개이며, 이 5개 모두 동일한 패턴 및 형식의 동적인 이벤트 기능을 가져야 한다.

// 2.만약 마우스가 A item 위에 있다가, B item으로 이동한다면, A는 원래 위치로 돌아오게 한다 B item은 위로 올라오게 하고싶다.
...
// 1.컴퓨터에게 상단 nav ul & li 태그를 인지시켜야 한다.

// 2.상단 nav 요소에게 마우스를 오버하면 이벤트가 실행.

//2_1.active가상클래스가 실행 (*텍스트 검정&두꺼워진다.)

//2_2.하단의 lnb요소의 opacity value 1이 되어야한다.

//3.mouseleave // mouseout 기능을 활용해서 마우스가 옆에 있는 li태그로 이동을 하면 기존 이벤트는 제거 // 신규 이벤트가 이전에 설정했던 기능을 그대로 구현!!
...
fetch("./data.json")
  ...
  .then((jsonData) => {
    ...
    contentDesc.innerHTML = firstData.description;

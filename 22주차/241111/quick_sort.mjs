...
// 공통 정의:
// 배열: 9개의 숫자를 가지고 있는 자료구조
// 배열의 0번째 인덱스 : 5 (*left)
// 배열의 8번째 인덱스 : 8 (*right)

// pivot : 기준 값 -> 배열의 0번째 인덱스.   위에서는 5
// 비교값 : 피봇을 제외한 나머지 배열 내 아이템 -> *비교값!
// 비교값 내 첫번째 인덱스 : leftStartIndex = 3
// 비교값 내 마지막 인덱스 : rightStartIndex = 8

// 규칙
// leftStartIndex 오른쪽으로 한 칸씩 이동, pivot보다 큰 숫자를 만나면 멈춤.
// rightStartIndex 왼쪽으로 한칸씩 이동, pivot보다 작은 숫자를 만나면 멈춤.
// 위의 두 인덱스 값이 조건에 충족되서 이동을 멈추게 되면, 두 숫자를 swap
// leftStartIndex롸 rightStartIndex가 서로 교차가 되는 시점이 오면, rightStartIndex 와 pivot의 값을 swap
